...
3. Remove the existing managed paths:
   ...
   - `~/.config/sketchybar`
   - `~/.skhdrc`
4. Stow the repository packages into `$HOME`.
...
The watcher observes only the repository packages, never the symlinked destination directories. It restows and restarts only the service affected by a change:

| Changed package | Action |
| --- | --- |
| `yabai/` | Restow and restart yabai |
| `skhd/` | Restow and restart skhd |
| `sketchybar/` | Restow and restart SketchyBar |
